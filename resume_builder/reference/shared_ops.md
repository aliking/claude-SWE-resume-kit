...
Standard workflow uses 3 sessions for token efficiency + quality:
...
> Used by `/make-resume` when a URL is provided instead of a file path.
> Perform this intake **before Phase 0**.
...
**Trigger:** User has made direct edits to the `.tex` resume file (confirmed via explicit user statement).
